var enjoyhint_steps = [
    {
        // 0
        "next .overview-title-wrapper":
            "최상단에는 <b>토의의 주제</b>가 있습니다.",
        showSkip: false,
        nextButton: { text: "다음" }
    },
    {
    ...
    {
        "next .overview-tabs":
            "<b>세 단계</b>를 거쳐 토의가 이루어집니다. 현재 '<b>문제 분석</b>' 단계에 있습니다.",
        showSkip: false,
        nextButton: { text: "다음" }
    },
    {
    ...
    {
        // 10
        "click .chatroom-utterances-wrapper.evidence":
            "특정 메시지 위에 마우스를 두면, 우측에 <b>'후보 등록'</b> 버튼이 나타납니다.<br>'후보 등록' 버튼을 <b>클릭</b>해서 이 메시지를 투표 후보로 올려보세요!",
        showSkip: false,
        onBeforeStart: function() {
...
enjoyhint_steps_moderator[8] = {
    "next .overview-section-container.current":
        "토의자들이 <b>투표를 진행</b>하였습니다. <br>의견이 어느 정도 모아진 것 같죠?<br>참고로 중복 투표가 가능합니다.",
    showSkip: false,
    onBeforeStart: function() {
        setTimeout(function() {
...
enjoyhint_steps_moderator.splice(13, 0, {
    "click .overview-next-button":
        "<b>'다음 단계' 버튼을 클릭</b>해서 <br>다음 질문으로 넘어가보세요.",
    showSkip: false,
    onBeforeStart: function() {
...
if (amModerator)
    enjoyhint_steps_moderator[7][
        "next #question0"
    ] = enjoyhint_steps_moderator[7]["next #question0"].replace(
        "현재는",
        "질문이 추가되었네요! 현재는"
    );
